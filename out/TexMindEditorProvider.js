...
exports.TexMindEditorProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const TexMindDocument_1 = require("./TexMindDocument");
...
class TexMindEditorProvider {
    ...
    resolveCustomTextEditor(document, webviewPanel, token) {
        ...
        if (!texMindDoc) {
            if (document.getText().trim()) {
                texMindDoc = TexMindDocument_1.TexMindDocument.parse(document.getText());
            }
            else {
                texMindDoc = new TexMindDocument_1.TexMindDocument((0, types_1.createDefaultDocument)());
            }
        ...
        webviewPanel.webview.onDidReceiveMessage(async (message) => {
            ...
            switch (message.type) {
                case 'update':
                    ...
                    const updatedDoc = message.document;
                ...
                case 'documentChanged':
                    ...
                    const changedDoc = message.document;
    ...
    async updateTextDocumentWithoutSaving(document, content) {
        const uriKey = document.uri.toString();
        this.applyingEditUris.add(uriKey);
        try {
            const edit = new vscode.WorkspaceEdit();
    ...
    // 导出 SVG
    async exportSvg(webviewPanel, svgContent) {
    ...
    // 导出 JSON
    async exportJson(document, content) {
        const uri = await vscode.window.showSaveDialog({
    ...
    getHtmlForWebview(webview) {
        ...
        // 获取资源文件 URI
        const mainJsUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'webview', 'dist', 'assets', 'main.js'));
        const mainCssUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'webview', 'dist', 'assets', 'main.css'));
        const katexCssUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'webview', 'public', 'katex.min.css'));
        // 直接构建 HTML，不依赖文件替换
        return `<!DOCTYPE html>
<html lang="en">
<head>
...
exports.TexMindEditorProvider = TexMindEditorProvider;
//# sourceMappingURL=TexMindEditorProvider.js.map